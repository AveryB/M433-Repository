...
void LCD_bar(int x, int y, int L, int w, short BarColorFront, short BarColorBack);
...
int main() {
 ...
 LCD_clearScreen(MAGENTA);
    ...
    LCD_drawString(msg, 20, 20, YELLOW, MAGENTA);
...
void LCD_bar(int x, int y, int L, int w, short BarColorFront, short BarColorBack) {
    int i = 0;
    for (i=x;i<x+L;i++) { 
        LCD_drawPixel(i,y,BarColorFront);
    }
    for (i = x+L; i< x+w; i++) {
        LCD_drawPixel(i,y,BarColorBack);
